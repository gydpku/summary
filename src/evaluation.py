...
def execute_sql(predicted_sql,ground_truth, db_path):
    ...
    print('ori',predicted_sql,'\n')
    predicted_sql=predicted_sql.split('final SQL')[1]
    predicted_sql='SELECT'+predicted_sql.split('SELECT')[1].split(';')[0]
    #predicted_sql='SELECT'+predicted_sql.split('SELECT')[1]
    #predicted_sql=predicted_sql.split('\n\n--')[0] if '\n\n--' in predicted_sql else predicted_sql #'SELECT'+predicted_sql.split('SELECT')[1] #print(dp_path) # Connect to the database
    
    #predicted_sql=predicted_sql.split(';')[0] if ';' in predicted_sql else predicted_sql #    print(dp_path)
    #predicted_sql=predicted_sql.replace('\\n',' ').replace('\\','')
    
    print('pred_sql',predicted_sql,'\n','ground',ground_truth)
    #time.sleep(10)
    cursor = conn.cursor()
#    pdb.set_trace()
    ...
    res = 0
#    time.sleep(25) #print('results',predicted_res,'truth',ground_truth_res,'\n')
...
def execute_model(predicted_sql,ground_truth, db_place, idx, meta_time_out):
    try:
#        pdb.set_trace() #    print(db_place,idx,'\n') #pdb.set_trace()
        res = func_timeout(meta_time_out, execute_sql,args=(predicted_sql, ground_truth, db_place)) #execute_sql(predicted_sql,ground_truth, db_place) #func_timeout(meta_time_out, execute_sql,args=(predicted_sql, ground_truth, db_place))
...
#    pdb.set_trace() # generate gt sqls:
