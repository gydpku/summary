...
def example_check(new_example,obj_passage):
        ...
        '''
        return new_example
def aug_few_shot_prompt_generate(high_quality_examples,task_instruction,obj_passage,operation_instruction=None,revise_instruction=None):
...
#    api_docs=""
    ...
    if len(new_examples)==0:
        return None,None
    return new_examples,new_examples[-1]
...
def cot_check_fill(demo_examples):
    ...
    if 'yes' in cot_status:
        return demo_examples
    else:
        for example in demo_examples:
            prompt="Your task is to generate the specific solution (step by step) to get its final output. The solution starts with 'Let's think step by step' and ends with 'The final answer is ...'".format(example)
...
def inference_data(instruction,domain,num,store_name,sample_num=3,passage_num=5000,types=None):
...
#    instruction='The domain is Medical. The TASK: Please classify the relationship between the given premise and hypothesis into one of the following labels: entailment, contradiction, or neutral. Return only the label.'
    ...
    try:
        ...
        torch.save(passages,'{0}.pt') #'medical_passages_sub.pt')
...
"This new example should belong to the same domain as the given example but be even more rare. The LENGTH and difficulty level of the new example should be similar to that of the given example."]
        ...
        all_example,new_example=aug_few_shot_prompt_generate(r_examples,task_instruction,sampled_objects,operation_instruction=op_instruction,revise_instruction=re_instruction)
...
#                pdb.set_trace()
        ...
        if all_example:
            synthetic_examples_all.extend(all_example)
