...
llm = LLM(model='/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/1k_op_valid_3_re_sql_model',gpu_memory_utilization=0.95) #(model='/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/dp_4.3k_no_semantic_sql_model',gpu_memory_utilization=0.95) #'/dccstor/obsidian_llm/yiduo/copy_v2/finetuned_models/dp_1k_no_semantic_sql_model',gpu_memory_utilization=0.95)

def connect_gpt(engine, prompt, max_tokens, temperature, stop):
    # print(prompt)
    import pdb
    prompt=prompt.replace('SELECT','') #replace(' after thinking step by step','').replace('SELECT','')
    prompt+=' To generate the SQL query to'
    try:
#        pdb.set_trace()
        output=llm.generate(prompt, sampling_params) #pdb.set_trace()
        result = output[0].outputs[0].text #query_azure_openai_chatgpt_chat(prompt) #openai.Completion.create(engine=engine, prompt=prompt, max_tokens=max_tokens, temperature=temperature, stop=stop)
        print(prompt,'\n Result',result)
